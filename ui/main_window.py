...
import threading
from src.obd_connector import OBDConnector  # Impor dari folder `src`
from config.config import load_config       # Impor dari folder `config`
...
class ECUScannerApp(QMainWindow):
    def __init__(self):
        ...
        if not self.config:
            QMessageBox.critical(self, "Error", "Gagal memuat konfigurasi. Pastikan 'config.yaml' tersedia.")
            return
    ...
    def initUI(self):
        ...
        try:
            ...
            print(f"Warning: Gagal memuat ikon: {e}")
